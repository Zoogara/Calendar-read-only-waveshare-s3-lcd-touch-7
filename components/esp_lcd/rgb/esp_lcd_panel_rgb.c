...
/* ---------------------------------------------------------------------
 * VENDORED + PATCHED COPY - forked from ESP-IDF v5.3.2's
 * components/esp_lcd/rgb/esp_lcd_panel_rgb.c (see
 * ~/esp/esp-idf/components/esp_lcd for the pristine original at that
 * version) into this project's own components/esp_lcd/, per ESP-IDF's
 * own documented override mechanism (a project-local components/<name>
 * takes priority over $IDF_PATH/components/<name> of the same name).
 *
 * Reason: this driver's VSYNC-triggered restart-recovery path
 * (lcd_rgb_panel_try_restart_transmission(), gated on by
 * CONFIG_LCD_RGB_RESTART_IN_VSYNC) always restarted the GDMA chain via a
 * single link hardcoded to fbs[0], regardless of which buffer was
 * actually current (cur_fb_index) - confirmed on real hardware as the
 * cause of a content-dependent flash of stale fbs[0] data whenever the
 * most recent redraw actually landed in a different buffer. Still present
 * on esp-idf's master branch as of this writing (2026-09), so this isn't
 * fixed in a newer release you could just upgrade to.
 *
 * Patch: dma_restart_link became an array (one per frame buffer, mirroring
 ...
 * manually re-applied here too - this copy won't pick them up automatically.
 * If Espressif ever fixes this upstream, drop this directory and go back to
 * the SDK's own copy.
 * --------------------------------------------------------------------- */
...
struct esp_rgb_panel_t {
    ...
    gdma_link_list_handle_t dma_restart_link[RGB_LCD_PANEL_MAX_FB_NUM]; // DMA link lists for restarting the DMA -
                                                     // PATCHED: one per frame buffer (see restart-link setup
                                                     // below) instead of a single link hardcoded to fbs[0]
...
static esp_err_t lcd_rgb_panel_destroy(esp_rgb_panel_t *rgb_panel)
{
    ...
    for (int i = 0; i < RGB_LCD_PANEL_MAX_FB_NUM; i++) {
        if (rgb_panel->dma_restart_link[i]) {
            gdma_del_link_list(rgb_panel->dma_restart_link[i]);
        }
    }
    if (rgb_panel->intr) {
...
static esp_err_t lcd_rgb_panel_init_trans_link(esp_rgb_panel_t *rgb_panel)
{
    ...
    if (rgb_panel->bb_size) {
        ...
        ESP_RETURN_ON_ERROR(gdma_new_link_list(&restart_link_cfg, &rgb_panel->dma_restart_link[0]), TAG, "create DMA restart link list failed");
        ...
        ESP_RETURN_ON_ERROR(gdma_link_mount_buffers(rgb_panel->dma_restart_link[0], 0, &restart_buffer_mount_cfg, 1, NULL),
        ...
        gdma_link_concat(rgb_panel->dma_restart_link[0], 0, rgb_panel->dma_bb_link, 1);
    } else {
        ...
        // create restart link
        // PATCHED (2026-09): upstream builds exactly one restart link here,
        // permanently mounted on fbs[0] and concatenated onto dma_fb_links[0]
        // regardless of num_fbs - see lcd_rgb_panel_try_restart_transmission(),
        // which (when CONFIG_LCD_RGB_RESTART_IN_VSYNC forces it to run every
        // VSYNC) always restarts the GDMA via this link. With num_fbs>1 that
        // means every VSYNC-triggered restart re-anchors the scan-out to
        // fbs[0] specifically, regardless of rgb_panel->cur_fb_index - i.e.
        // whichever buffer was actually just drawn into. Confirmed on real
        // hardware (Waveshare ESP32-S3-Touch-LCD-7, IDF v5.3.2) as the cause
        // of a content-dependent flash of stale fbs[0] data. Fix: build one
        // restart link PER frame buffer (mirroring the dma_fb_links[] loop
        // just above), each concatenated onto its OWN buffer's link list, and
        // select by cur_fb_index at restart time instead of always using
        // index 0. Filed upstream: TODO add espressif/esp-idf issue link.
        gdma_link_list_config_t restart_link_cfg = {
            .buffer_alignment = rgb_panel->flags.fb_in_psram ? rgb_panel->ext_mem_align : rgb_panel->int_mem_align,
            .item_alignment = LCD_GDMA_DESCRIPTOR_ALIGN,
            .num_items = 1, // the restart link only contains one node
        ...
        for (size_t i = 0; i < rgb_panel->num_fbs; i++) {
            ESP_RETURN_ON_ERROR(gdma_new_link_list(&restart_link_cfg, &rgb_panel->dma_restart_link[i]), TAG, "create DMA restart link list failed");
            gdma_buffer_mount_config_t restart_buffer_mount_cfg = {
                .buffer = rgb_panel->fbs[i] + restart_skip_bytes,
                .length = MIN(LCD_DMA_DESCRIPTOR_BUFFER_MAX_SIZE, rgb_panel->fb_size) - restart_skip_bytes,
            };
            ESP_RETURN_ON_ERROR(gdma_link_mount_buffers(rgb_panel->dma_restart_link[i], 0, &restart_buffer_mount_cfg, 1, NULL),
                                TAG, "mount DMA restart buffer failed");

            // Magic here: we use the restart link to restart the frame buffer link list, so concat them -
            // PATCHED: each restart link concats onto ITS OWN buffer's chain (i, not always 0), so restarting
            // mid-stream always resumes into whichever buffer this particular restart link corresponds to.
            gdma_link_concat(rgb_panel->dma_restart_link[i], 0, rgb_panel->dma_fb_links[i], 1);
        }
    }
